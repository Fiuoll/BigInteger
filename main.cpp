...
//  }
  ...
  printf ("  1 - use BigInt (supported operations: +, -, *)\n");
  printf ("  2 - use ModUInt (supported operations: +, *)\n");
  printf ("  3 - use ModUInt with OpenCL (supported operations: +, *)\n");
  printf ("  0 - use all methods (supported operations: +, *)\n");

  scanf ("%s", buf1);
  int method = atoi(buf1);
  if (method != 0 && method != 1 && method != 2 && method != 3)
    return 0;
  int m_opencl = method == 3 || method == 0;
  int m_biguint = method == 1 || method == 0;
  int m_moduint = method == 2 || method == 0;
  ...
  while (1)
    ...
    #pragma GCC diagnostic pop
          ...
          dt = (clock() - dt) ;
          printf ("Kernel operation time: %.5f ms\n", dt * 10e-3);
    ...
    #endif
              if(    (cl_flag && (a[i] * b[i]) % primes[i] != c[i] / 100)
                  || (!cl_flag && (a[i] + b[i]) % primes[i] != c[i]))
                {
                  fprintf(stdout,"error [%i]\t\t. %.1ld\t+ %.1ld\t %.1d\t != \t%.1ld\n", i, a[i], b[i], primes[i], c[i]);
                  er = 1;
                  break;
                }
            }
